...
@section('title', 'Coins | CryptoGainers')
                                    ...
                                    @foreach ($coins as $coin)
                                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                            ...
                                            <td
                                                ...
                                                <a href="{{ route('coins.show', $coin->slug) }}" target="_blank"
                                                    class="flex">
                                                    <p class="truncate w-40"> {{ $coin->name }} ({{ $coin->symbol }})
                                                    </p>
                                                </a>
                                            </td>
                                            <td
                                                ...
                                                <div class="relative flex justify-center items-center ">
                                                    ...
                                                    @can('delete', $coin)
                                                        <button @click.prevent="setDelete($event)"
                                                            data-deleteId="deleteNewsForm{{ $coin->id }}"
                                                            class=" lg:text-sm lg:leading-6  font-medium text-slate-700 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-300">
                                                            <div
                                                                class="pointer-events-none mx-1 bg-gray-50/5 rounded-md ring-1 ring-slate-900/5 shadow-sm group-hover:shadow group-hover:ring-slate-900/10 dark:ring-0 dark:shadow-none dark:group-hover:shadow-none dark:group-hover:highlight-white/10 group-hover:shadow-blue-200 dark:group-hover:bg-blue-500 dark:bg-slate-400/10 dark:highlight-white/5">
                                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                        ...
                                                        <form id="deleteNewsForm{{ $coin->id }}"
                                                            action="{{ route('admin.users.create', $coin) }}" method="POST">
                                                            @csrf
                                                            @method("DELETE")
                                                        </form>
                                                    @endcan
                                            ...
                                            <td
                                                class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                <p class="truncate w-40"> {{ $coin->slug }}</p>
                                            </td>
                                            <td
                                                class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                ...
                                                @foreach ($coin->tags as $tag)
                                                    @if ($tag_counter < 1)
                                                        <a href="{{ route('news.index') }}/{{ $coin->slug }}?tag={{ $tag }}"
                                            ...
                                            <td
                                                ...
                                                {{ _ago($coin->created_at) }}
                                                {{-- {{ _ago($coin->updated_at) }} --}}
                                            </td>
                                        </tr>
                                    @endforeach
